...
Før der gåes i detaljen med de forskellige udtryk er det nødvendigt at omtale brugen af operatorer og operatorprioritering.
...
\noindent Herunder ses semantikken for \textit{And expression}. Hvis der tages udgangspunkt i $[AND_\top]$ skal semantikken læses således: Hvis $e_1$ evaluerer til \textit{true} i $SEnv$ og $e_2$ evaluerer til \textit{true} i $SEnv$ så evaluerer udtrykket $e_1 || e_2$ til true i $SEnv$. 

Hvis der tages udgangspunkt i $[AND_\bot]$ skal semantikken læses således: Hvis $e_i$ evaluerer til \textit{false} i $SEnv$  for i værende 1 eller 2, så evaluerer udtrykket $e_1 && e_2$ til false i $SEnv$.

nv$ så evaluerer udtrykket $e_1 || e_2$ til true i $SEnv$. 
