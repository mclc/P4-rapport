...
Datatyper skal forstås som de basale datatyper (såsom \textit{int}, \textit{float} …), som sproget skal indeholde. PLC++'s typesystem er meget sammenligning med C med et par få tilføjelser, disse tilføjelser er \textit{timer}, \textit{port} samt \textit{bool} (se afsnit \ref{sec:forskelfrac} \mfix{afsnit findes ikke længere}).
...
For at give programmøren mulighed for at foretage matematiske beregninger i sproget (som var muligt i \gls{st} og {il} – se afsnit \mefix{indsæt reff}), skal PLC++ som minimum understøtte de basale aritmetiske operationer. Heriblandt implementeres: minus ('-'), plus ('+'), dividere ('/') samt gange ('*').
...
Dette afsnit vil på baggrund af det foregående kapitel \mefix{indsæt reff} (problemanalyse) og afsnit \mefix{indsæt reff} (valg af paradigme) og \mefix{indsæt reff} (MoSCoW), udspecificere de relevante kriterier og belyse rationalet, som er anvendt til at prioritere sprogets kriterier. Afslutningsvist vil kriterierne blive opsat i en tabel, som giver et samlet overblik. 
